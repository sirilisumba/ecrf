...
import time
import subprocess
from ecrfv2_utils import create_driver, load_excel_data, set_text, isi_radio_button, isi_date_indo, set_radio, save_form, play_sound, save_form1, isi_dropdown
...
def jalankan_script(attempt=1, max_attempts=3):
    ...
    try:
        ...
        try:
            ...
            # dropdown = wait_short.until(EC.element_to_be_clickable((By.ID, "puskesmas")))
            # Select(dropdown).select_by_visible_text(data['val_puskesmas'])
            # print(f"→ puskesmas = {data['val_puskesmas']}")
            isi_dropdown(driver, wait_short, "puskesmas", data['val_puskesmas'])
            ...
            subprocess.run(["python", "ecrfv2-2serius.py"])
