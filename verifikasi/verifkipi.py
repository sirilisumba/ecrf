...
from verif_utils import (
    ...
    get_label_text, tulis_verif, cek_dan_klik_verified_smart, play_error
...
try:
    ...
    ws.delete_rows(3)
    wb.save(EXCEL_PATH)
    print("🗑️ Baris 3 dihapus")
    wb.close()
    log_wb.close()


    # ---  LANJUT KE DATA / NOMOR INKLUSI BERIKUTNYA 
    next_file = "verif.exe" if getattr(sys, 'frozen', False) else "verif.py"
    
    ans = input("👉 Tekan ENTER untuk lanjut, atau ctrl+C untuk keluar: ").strip().lower()
    if ans != "stop":
        print("▶️  Next DATA...")
